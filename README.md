...
- **画像生成AIで作成した素材です。使用したのはOpenAIのGPT Imageです。**
  プロンプトは[背景画像 生成プロンプト集](docs/background-prompts.md)、
...
> ### ⚠ 公開前に必ず確認すること
>
> 背景にGPT Image（OpenAI）の出力を使用しています。これは法的助言ではないので、
> 必ず一次情報で判断してください。
>
> **確認済みとみている点**
>
> OpenAIのTerms of Useは出力物の権利を利用者へ譲渡する規定を置いており、
> 商用利用・再配布を禁じていません（`Ownership of content`の節）。
> ただし出力が第三者の権利を侵害しないことは保証されません（無保証条項）。
> **提出前に原文で再確認してください。**
>
> **未解決の点**
>
> 1. **Zepp側のAI生成アセットに関する方針**。Zepp Developer Service Agreementと
>    審査ガイドラインに、AI生成素材の可否や開示義務の定めがあるかを確認すること。
>    これが最も直接的なリスクで、本リポジトリでは未確認。
> 2. **著作権の成立**。純粋なAI生成物は人間の著作性を欠くとして保護が
>    認められない扱いが各国で示されている。配布の可否とは別の論点だが、
>    「独自制作物」としての申告内容に影響する。
>
> 背景以外（数字、記号、天候アイコン、レイアウト、コード）は本プロジェクトの
> オリジナルです。申告の際はこの区別を保ってください。
