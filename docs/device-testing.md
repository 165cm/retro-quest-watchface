...
4. Simulatorの **Sensors** パネルで以下を切り替えて確認する
   ...
   - **Battery** — 100% / 68% / 45% / 15% / 0%（HPの棒の長さと色が変わる。緑→黄→赤）
   - **Weather** — index を変えて背景と天候アイコンが連動するか
   - **Step** — 歩数が反映され、3桁区切りが入るか
...
3. **Background preview (debug)** のセクションが表示される ← これを使う
...
- [ ] 上のパネル: 天候アイコン・日付・現在気温が重ならず、**左右も見切れていない**
- [ ] **日付とHPラベルの間隔**（システムフォントの字幅が想定と違わないか）
- [ ] 時刻が全10テーマで読める（`Cycle`で一周させる）
- [ ] **Snow / Fog** で白い時刻数字が背景に埋もれていない
- [ ] 中央の開放部で**背景がしっかり見えている**
- [ ] 下段: 気温3列（L / NOW / H）と歩数が重ならず、左右も見切れていない
- [ ] 歩数の足あとアイコンが潰れずに判別できる

### 機能

- [ ] 12時間制で **AM/PM** が表示され、時刻数字と重なっていない
- [ ] **数字列が画面中央にある**（AM/PMは中央寄せの計算に含めない仕様）
- [ ] 1桁時（`9:05`）と2桁時（`10:38`）の両方で中央寄せが崩れない
- [ ] 歩数が実際の値を示し、3桁区切りが入っている
- [ ] 日付が `M/D DDD` 形式（曜日は英語大文字）
- [ ] 気温 L / NOW / H が実際の天気の値になっている
- [ ] 摂氏／華氏の切り替えに追従する
- [ ] バッテリー残量でHPの棒の長さと色が変わる（緑→黄→赤）、右端の%と一致している
